...
export function StatusBanner({ variant, title, description, actions, onDismiss }: StatusBannerProps) {
  ...
  return (
    <div className={cn("swiss-card flex flex-col gap-3 rounded-lg border px-4 py-4", classes.container)}>
      <div className="flex items-start gap-3">
        <div className={cn("flex h-10 w-10 items-center justify-center rounded-lg", classes.icon)}>{icon}</div>
        <div className="flex-1 space-y-1">
          ...
          {actions && actions.length > 0 ? (
            <div className="flex flex-wrap items-center gap-2 pt-1">
              {actions.map((action) => (
                <Button key={action.label} size="sm" variant="outline" className="border-ink text-ink hover:bg-ink hover:text-white transition-colors font-medium" onClick={action.onClick}>
                  {action.label}
                </Button>
              ))}
            </div>
...
function getClasses(variant: StatusVariant) {
  switch (variant) {
    case "success":
      return {
        container: "border-sage bg-sage-light/50",
        icon: "bg-sage text-white",
      };
    case "warning":
      return {
        container: "border-amber bg-amber-light/50",
