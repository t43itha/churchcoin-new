...
export function PledgeUploadCard({ onFileParsed }: PledgeUploadCardProps) {
  ...
  return (
    <Card className="border-ledger bg-paper shadow-none">
      <CardHeader>
        <CardTitle className="text-ink">Upload pledge list</CardTitle>
        <CardDescription className="text-grey-mid">
          Import pledges in bulk from CSV. We&apos;ll match donors and preview before creating.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {error ? (
          ...
          <p className="text-xs text-grey-mid">
            Expected columns: Donor Name (required), Donor Email, Amount, Pledged Date, Due Date, Notes
          </p>
        )}
      </CardContent>
    </Card>
